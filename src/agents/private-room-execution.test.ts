...
describe("private room execution authority", () => {
  it("binds one authenticated root to an exact instance, session, and live owner", async () => {
    ...
    const execution: PrivateRoomExecution = {
      ...
      inputMessageId: "committed-human-input",
      assertCurrent: () => {
        if (!active) {
    ...
    await withPrivateRoomExecution(execution, async () => {
      ...
      expect(
        privateRoomExecutionForRun({ ...instance, instanceId: "replacement" }),
      ).toBeUndefined();
      expect(() => assertPrivateRoomExecutionTarget({ sessionKey: "agent:main:another" })).toThrow(
        "another session",
      );
      expect(() =>
        assertPrivateRoomExecutionTarget({ sessionKey: execution.sessionKey, sessionId: "old" }),
