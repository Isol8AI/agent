...
- `gateway.auth.trustedBrokerProfiles`: optional token-mode binding from an exact paired device ID to an existing canonical non-owner profile ID. It is for a trusted server backend that holds the Gateway token and one distinct Ed25519 device key per represented human. The backend sends `auth.trustedBrokerProfileId`; device signature payload v4 binds that value to the challenge nonce, token, device, client, role, scopes, and platform metadata. The Gateway accepts it only for the `gateway-client` / `backend` operator client without a browser `Origin`, only when the mapped device and asserted profile match exactly, and only after ordinary paired-device authorization. A mapped device without the assertion fails closed. The assertion does not add scopes; configured profile roles still cap the connection. Never send the Gateway token, signing key, or assertion from a browser.
