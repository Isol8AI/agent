...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      // Trusted server brokers only: paired device ID -> existing human profile ID.
      // trustedBrokerProfiles: { "<64-character-device-id>": "<profile-id>" },
      rateLimit: {
        maxAttempts: 10,
        windowMs: 60000,
        lockoutMs: 300000,
...
- `gateway.auth.trustedBrokerProfiles`: optional token-mode binding from an exact paired device ID to an existing canonical non-owner profile ID. It is for a trusted server backend that holds the Gateway token and one distinct Ed25519 device key per represented human. Before writing the binding, an authenticated `operator.admin` may call `users.ensureProfile` with exactly `{ alias: "clerk-<64-lowercase-hex>@identity.lightbulb.invalid" }`; this idempotently returns the canonical profile and accepts no caller-selected ID, role, scopes, display name, or ordinary email. The backend sends `auth.trustedBrokerProfileId`; device signature payload v4 binds that value to the challenge nonce, token, device, client, role, scopes, and platform metadata. Its canonical bytes begin with `v4|`; every following field is `<decimal UTF-8 byte length>:<raw value>`, separated by `|`, in this order: device ID, client ID, client mode, role, decimal scope count, each scope in sent order, signed-at milliseconds, token, nonce, normalized platform, normalized device family, profile ID. The Gateway accepts it only for the `gateway-client` / `backend` operator client without a browser `Origin`, only when the mapped device and asserted profile match exactly, and only while the exact key, role, and scopes have an existing live paired-device authorization. Missing, removed, or changed pairing state fails without silent local repair. A mapped device without the assertion fails closed. The assertion does not add scopes, and the connection receives no reusable device or bootstrap token; configured profile roles still cap it. Never send the Gateway token, signing key, or assertion from a browser.
